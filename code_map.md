...
│     ├─ i18n/                  # 自建轻量国际化（v1.0.26）：index.tsx（I18nProvider/useI18n/t，fallback lang→zh→key）+ zh.ts/ko.ts/en.ts 扁平词典
...
| job_scheduler | `JobScheduler` | jobs.json | create / list / cancel / delete（v1.0.20，彻底移除记录含已 done/cancelled）/ _tick（1s，pending/active→done/cancelled）。**有 prompt 的任务到期后经 task_runner 真实执行，结果写回 job.result**；空 prompt = 仅留痕提醒 |
...
| GET / POST / DELETE | `/jobs`、`/jobs/{id}/cancel`、`/jobs/{id}` | 定时任务列表 / 取消任务 / 删除任务记录（含已 done/cancelled 留痕，v1.0.20） |
...
- **i18n（v1.0.26，自建轻量、无新依赖）**：`web/src/i18n/` = `index.tsx`（`I18nProvider` 用 Context 暴露 `{lang,setLang,t}`；`t(key,vars)` 查扁平词典，缺失回退 `zh` 再回退 key 本身，`{name}` 占位用 split/join 插值）+ `zh.ts`/`ko.ts`/`en.ts`（key 一致的扁平 `Record<string,string>`，zh 为源须齐全）。`App.tsx` 持有 `lang` 状态并用 `I18nProvider` 包裹整棵树（**App 自身在 Provider 之上，不能用 useI18n**，故 App 内文案如移动端 FAB tooltip 暂未翻译）；初始 lang 从 `settings.ui.language` 读取，`handleLangChange` 即时 setState + `PUT /api/settings {ui:{language}}` 持久化。**语言选择器**在 `SessionList` 的 `.sidebar-foot` 内、齿轮按钮**上方**（`.sidebar-foot` 改 `flex-direction:column`），是独立于 `.session-list` 任务容器的 `<select>`（GlobeIcon + 中文/한국어/English）。**已迁移**（三语实时切换、浏览器实测）：SessionList 侧边栏全部、SettingsPanel 菜单、SettingsShell 返回、ChatArea 全部外壳（工具卡/步骤时间线/EvalStrip/StageMetrics/PipelinePanel/ThinkingTrace+traceSummary/ConfirmDialog/AttachmentChip/头部/空状态/输入区/脚注）；ChatArea 内 `evalTitle`/`traceSummary` 为纯函数故显式传 `t`，`FALLBACK_REASON` 改为 key 映射 `FALLBACK_REASON_KEY`+`reasonText(fb,t)`，各子组件 `useI18n()` 必须在早返回**之前**调用（遵守 Hooks 规则）。**未迁移（增量待补，见 §9 v1.0.26）**：11 个设置子页 + LibraryPanel/WorkspacePanel/AutomationPanel/ToolPanel + 各 Modal（仍硬编码中文，切语言时这些深层面板不变）。**后端**：`config.py` 默认 `ui.language="zh"`、`settings.py` `_UI_KEYS` 加 `"language"`（属 app/ 改动，已重启生效）。
...
| `verify.py` | 离线端到端验收 14 项（S0-S13，fixed/fast/standard/deep/工具/评估返工/job 真实执行/记忆/页面/回归/边界/RAG/automation/认证开关）；S0 后自动 PUT mock 角色模型（mock-planner/executor/evaluator/memory），跑前需先起 mock（4831）。**自快照/自恢复**（v1.0.19）：切换前 `snapshot_settings()` 读 config.json 原文存底，`__main__` 的 `try/finally` 保证无论正常结束、断言失败还是 Ctrl-C 都 `restore_settings()` 经 PUT 精确还原（同步刷新服务端缓存、免重启），杜绝崩溃后残留 mock 坏配置 |
...
| 版本 | 时间 | 修改人 | 内容摘要 |
|---|---|---|---|
| v1.0.26 | 2026-09-26 | Qoder | **全应用 i18n：设置按钮上方加语言选择器，中/韩/英实时切换 + 后端持久化**（前端为主 + 后端 2 处）。用户交互规则：设置按钮上方增加语言选择、独立于任务履历容器之外，下拉含中文/韩文/英文，选中后页面整体内容实时切换。**经 AskUserQuestion 定三事**：①实现=自建轻量 i18n（无新依赖）；②范围=先核心界面其余增量补；③持久化=存后端 `settings.ui.language`。**新增 `web/src/i18n/`**：`index.tsx`（`I18nProvider` Context 暴露 `{lang,setLang,t}`；`t(key,vars)` 查扁平词典、缺失回退 zh 再回退 key、`{name}` 用 split/join 插值；`LANGS`/`isLang`）+ `zh.ts`/`ko.ts`/`en.ts`（key 对齐的扁平词典，zh 为源）。**接线**：`App.tsx` 持 `lang` 状态、`I18nProvider` 包裹整棵树、初始从 `settings.ui.language` 读、`handleLangChange` 即时生效 + `PUT /api/settings {ui:{language}}`；`types.ts` `UIConfig` 加 `language?`。**选择器**：`SessionList` `.sidebar-foot` 改 `flex-direction:column`，齿轮上方放 `<select>`（GlobeIcon + 中文/한국어/English），天然独立于 `.session-list`。**已迁移三语**（浏览器实测实时切换）：SessionList 侧边栏全部（导航/任务/显示更多/设置/返回应用/relTime/tooltip/confirm）、SettingsPanel 菜单（4 组 + 10 条目 + 副标题）、SettingsShell 返回、ChatArea 全部外壳（ToolCard/StepTimeline/EvalStrip/StageMetrics/PipelinePanel/ThinkingTrace+traceSummary/ConfirmDialog/AttachmentChip/头部/空状态/输入区/脚注，约 60 键）；纯函数 `evalTitle`/`traceSummary` 显式传 `t`、`FALLBACK_REASON`→key 映射 `FALLBACK_REASON_KEY`+`reasonText`，各子组件 `useI18n()` 置于早返回**之前**（遵守 Hooks 规则，迁移中两次纠偏）。**后端**（需重启，已重启 4830）：`config.py` 默认 `ui.language="zh"`、`settings.py` `_UI_KEYS` 加 `"language"`。**验证**：`vite build` OK（bundle `index-NOIfNOfL.js` 449.67 kB / `index-BaHgLdt7.css` 89.48 kB）、`npx tsc --noEmit` 零报错（捕获并修正 StageMetrics 里 `s.fallback` 可能 undefined 与 vars 类型不符，改 `String(s.fallback)`）；`PUT {ui:{language:ko}}`→`GET` 回显 ko（白名单生效）；浏览器实测——配置 ko 时刷新侧边栏直接韩文（从后端初始化）、选择器值 ko 且在齿轮上方（`selAboveGear=true`、foot 子元素序 `[lang-select,gear-btn]`）、切 en 实时变英文（导航/分组/条目/返回应用/聊天脚注/占位符/发送 title）、切 ko 实时变韩文（含占位符「메시지 입력...」，首测因捕获时序读到旧值、重测 600ms 确认）、切回 zh 复原，后端 language 随每次切换持久化（最终复位 zh）。**未迁移（增量待补，§4.1 已记）**：11 设置子页 + Library/Workspace/Automation/ToolPanel + 各 Modal 仍硬编码中文，切语言时深层面板不变；App 自身文案（移动端 FAB tooltip）因 App 在 Provider 之上未译。**文档**：文件树补 `i18n/`、§4.1 新增 i18n 条、§9 本行、§10 刷新 bundle 行。 |
| v1.0.25 | 2026-09-26 | Qoder | **侧边栏任务履历默认只显示最近 5 条，其余折叠隐藏**（纯前端）。用户交互规则：任务履历列表默认只加载显示 5 条，超出的全部折叠隐藏，收纳形式参考最近任务折叠样式。**背景**：`session_manager.list_sessions()` 已按 `updated_at` 倒序返回，故前 5 即最近 5 个任务；此前 `SessionList.tsx` 无条件 `sessions.map` 渲染全部（实测 live 11 个会话全铺开）。**改动**（`SessionList.tsx` + `styles.css`）：① 新增 `showAllTasks` state（默认 false）+ `TASK_LIMIT=5`/`hiddenCount`/`visibleSessions=showAllTasks?sessions:sessions.slice(0,5)`，列表改渲染 `visibleSessions`；② 仅当 `sessions.length>5` 时在列表末尾渲染「显示更多 (N) / 收起」折叠行 `.session-more`（点击切 `showAllTasks`），视觉参照既有任务分组头 `.nav-group-head`（flex 两端对齐 + 12px 灰字 + hover 变色），chevron 用 `.more-chevron`（展开时 `.up` 旋转 180°）；③ 新增 `.session-more`/`.session-more:hover`/`.more-chevron(.up)` CSS。折叠行在 `.session-list` 内，故随「任务(N)」分组头整体折叠时一并隐藏，与 v1.0.24 的齿轮底部锚定不冲突。**验证**：`vite build` OK（bundle `index-BEuNGG_L.js` 429.05 kB / `index-CiqhzrMW.css` 88.71 kB）；纯前端无需重启（后端 4830 serve dist，实测 `index.html` 已引用新哈希）。浏览器 `evaluate_script` 实测（live 11 会话）：默认 **5 张卡 + 「显示更多 (6)」**；点展开 → **11 张卡 + 「收起」+ chevron 朝上**；点收起 → 回 **5 张 + 「显示更多 (6)」**，双向切换正常（其间一次 250ms 测量撞上 React 重渲染瞬态，重测 500ms 确认折叠生效）。**文档**：§9 本行、§10 刷新 bundle 行。 |
| v1.0.24 | 2026-09-26 | Qoder | **设置（齿轮）按钮固定锚定侧边栏底部，不随任务列表折叠/展开上移**（纯前端）。用户交互规则：侧边栏【任务履历】折叠/展开切换时，设置按钮位置固定、始终锚定在侧边栏右下角，不受列表折叠展开影响。**根因**：`.sidebar` 是 flex 列，展开态靠 `.session-list{flex:1}` 撑满把 `.sidebar-foot`（齿轮容器）顶到底部；点「任务(N)」分组头折叠时 `.session-list.hidden{display:none}` 移除该撑开元素，无 flex:1 占位 → `.sidebar-foot` 紧随 `.nav-group-head` 上移。**改动**（`styles.css`）：给 `.sidebar-foot` 加 `margin-top:auto`——flex 列里恒定把它推到主轴末端（底部），与既有 `justify-content:flex-end` 合成右下角锚定；展开态 session-list 已占满剩余空间则 auto 边距解析为 0，无副作用，故展开/折叠/设置面板三态一致。**验证**：`vite build` OK（bundle `index-w4D0URck.js` 428.72 kB / `index-BAE9Hst9.css` 88.30 kB）；纯前端无需重启（后端 4830 serve dist，实测 `index.html` 已引用新哈希）。浏览器 `evaluate_script` 实测三态齿轮 `gearTop` 恒为 **748**、距侧边栏底 **8px**：展开（listHidden=false）→ 折叠（listHidden=true）→ 再展开，位置零漂移。**文档**：§9 本行、§10 刷新 bundle 行。 |
| v1.0.23 | 2026-09-26 | Qoder | **设置子页面内容限宽 50% + 水平居中，不再全屏铺开**（纯前端）。用户交互规则：在设置功能页面点击左侧侧边栏任意菜单条目后，右侧主区域内容不要全屏铺开，容器限定为页面 50% 宽度、水平居中，布局参照现有任务页面样式。**根因**：v1.0.18 起设置子页面在主区域平铺，`.settings-page-container .setpage` 只有 `height:100%/min-height:100%`、宽度默认撑满容器。**改动**（`styles.css`）：给 `.settings-page-container .setpage` 加 `width:50%` + `margin:0 auto`，复用任务页 `.chat-col`（`max-width:820px;margin:0 auto`）同款居中列思路；选择器作用于容器内所有 `.setpage`，故 11 个设置子页面统一生效，无需逐页改。**验证**：`vite build` OK（bundle `index-B0WlDCNm.js` 428.72 kB / `index-B2ZGYap-.css` 88.28 kB）；纯前端无需重启（后端 4830 serve dist，实测 `index.html` 已引用新哈希）。浏览器 `evaluate_script` 实测两个不同子页面——「主题与外观」与「LLM 设置」：`.setpage` 宽度 / `.settings-page-container` 宽度比值均 **0.5**，左右间距相等（**centered=true**，如容器 531px→内容 266px、左右各 133px），内容不再铺满主区域。**文档**：§9 本行、§10 刷新 bundle 行。 |
| v1.0.22 | 2026-09-26 | Qoder | **设置侧边栏顶部新增「← 返回应用」导航项**（纯前端）。用户要求：点击设置按钮打开左侧侧边栏后，在所有菜单最上方增加一个「← 返回应用」条目，箭头与文字保持合理间距，点击后回到应用主页面。**改动**：① `SettingsPanel.tsx` 新增 `onBackToApp` prop + `BackArrowIcon`（左向箭头 SVG），在 `.settings-side` 内 `groups.map` 之前渲染 `.sm-back` 按钮（`.sm-back-arrow` + `.sm-back-text`「返回应用」），确保位于四个分组（个性化/模型与连接/智能体与自动化/知识与上下文）之上、为侧边栏首个子元素；② `SessionList.tsx` 接口与解构补 `onBackToApp`，透传给 `SettingsPanel`；③ `App.tsx` 将已有的 `closeSettings`（`setSettingsPage(null)` + `setLeftSettingsOpen(false)`）作为 `onBackToApp` 传入——点击即同时清除设置子页面并收起设置侧边栏，回到主界面（side-nav + ChatArea）；④ `styles.css` 新增 `.sm-back`（flex / gap:8px / 复用 `.sm-item` 的边框+圆角+阴影视觉）、`.sm-back-arrow`、`.sm-back-text` 及暗色主题 `.sm-back:hover`。**验证**：`vite build` OK（bundle `index-CKpp75za.js` 428.72 kB / `index-Oabpg6RV.css` 88.26 kB）；纯前端改动无需重启（后端 4830 serve dist，实测 `index.html` 已引用新哈希）。浏览器 `evaluate_script` 实测端到端通过——① 点齿轮 → 侧边栏出现，`.settings-side.firstElementChild` 为 `.sm-back`（在「个性化」分组之前）、文字「返回应用」、含箭头 SVG；② 点「主题与外观」进子页面 → 点「返回应用」→ `settings-side`/`settings-page-container`/`setpage` 全部消失、`side-nav`+`nav-new`+聊天区恢复、齿轮失去 `.active`；③ 箭头 16px 宽 + 8px 间距 + 13px 文字，间距合理。**文档**：§9 本行、§10 刷新 bundle 行。 |
| v1.0.21 | 2026-09-26 | Qoder | **修 verify.py S11 拒答探针，离线验收回到 14/14 全 PASS**。v1.0.20 跑出 S11 FAIL，确诊为**测试自污染**（非产品缺陷、确定性复现）：S11 拒答探针用英文 `quantum entanglement bell inequality`，与同轮 S2/S7 注入的短英文 mock 页 `hello from mock executor` 共享 bigram（`el`/`ll`），短文档 BM25 长度归一化把 `best_bm25/_BM25_CONF_CAP(6.0)` 顶过 `refuse_threshold=0.35` → `answer()` 未拒答；向量路径无辜（实测 cos 全 <0.35），对干净 119-chunk 索引同查询 `conf=0.0` 正确拒答。修法（`scripts/verify.py` S11）：探针改中文无关查询「量子纠缠贝尔不等式」，并加注释说明为何不能用英文探针。**改前实测验证**：中文探针 BM25 conf=0.0、mock-embed 对真实中文页 max cos=0.25 / 对英文 mock 页 cos=0.00，均 <0.35 留有裕度。**验证**：`py_compile` OK；完整跑 `scripts/verify.py` → **14/14 PASS、exit 0**；跑完逐字段 diff before/after 仍 **ZERO RESIDUE=True**（sessions/pages/jobs/memories/automations/hello.txt/rag(embed='',119)/config 全还原），自清扫删 9 项（3 会话含 2 个 task_runner `[自动]` + 2 页 + 1 任务 + 2 记忆 + hello.txt）。**文档**：§6 离线验收行更新为 14/14 + 探针修复、§9 本行、PATCH_NOTES「已知失败」段改写为「S11 修复（v1.0.21）」。纯脚本改动，**无需重启**。 |
| v1.0.20 | 2026-09-25 | Qoder | **verify.py 数据残留自清扫 + base_url 自切换 + 新增 DELETE /api/jobs/{id}；清理 live data 历史 mock 污染**。承接 v1.0.19（settings 自快照/自恢复）。**起因**：用户跑完整 verify 后发现 `/api/rag/search` 仍报 `embed_model='mock-embed'`、pages 里有 mock 测试页——证实 verify 跑在 live 服务上会留下 settings 之外的多类残留，且 S11 的 reindex 会用 mock 网关把 live RAG 索引整个重建成 mock 向量。**根因**：verify.py 是独立进程，只能经 HTTP 操作服务端；而 `RagEngine`（`_loaded`/`_chunks`/`_embed_model`/`_signature`）与模块级 `_detect_cache`、`JobScheduler._jobs` 均为服务端进程内缓存，改磁盘文件运行中的服务读不到——故「快照还原 rag_index.json」对 live 服务无效（且删 mock 页会改语料签名触发 `_ensure_fresh` 自动重建覆盖还原）。**改动**：① `scripts/verify.py` 新增 `MOCK_GATEWAY`/`MOCK_ORIGIN`，`MOCK_SETTINGS` 补 `base_url` 指向 mock 网关、`restore_settings` 同步还原 `base_url`（此前 verify 只改 model/roles 不改 base_url，需手工切换才连得上 mock，是「靠人记配置」脆弱性的另一半）；② 新增 `capture_baseline()`（跑前抓 sessions/pages/jobs/memories 既有 id）+ `cleanup_residue()`（finally 调用）：按「基线后新增」删会话与记忆（task_runner 派生的 `[自动]` 会话与 auto-extract 文案不可枚举，故全删新增），页面/任务额外要求 mock 标识（`mock 页面`/`workspace:hello.txt`/`hello from mock` 前缀、job 标题 `mock` 前缀）避免误删并发真实数据，删 `workspace/hello.txt`，最后 `POST /api/rag/reindex` 复位 `_detect_cache` 并重建干净索引；③ `app/job_scheduler.py` 新增 `delete(job_id)`、`app/routes/extras.py` 新增 `DELETE /api/jobs/{id}`（此前 job 无删除路径，verify 无法自清 S5 创建的任务，UI 也删不掉已结束任务）。**live data 清理（本轮实测）**：经 API 删 3 条 mock 记忆（`用户喜欢蓝色`/`用户偏好简洁回答`/`去重测试条目`）+ 2 个 mock 页（`hello`/`mock 页面`）+ 2 个 verify 会话；9 个 `mock 提醒` job 无 DELETE 端点→直接改 `jobs.json`（备份 `data/cache/jobs.json.pre-mock-clean`）+ 重启刷新调度器内存；`/api/rag/reindex` 把索引从 `mock-embed`/121chunks 重建为 `(bm25)`/119chunks（内网代理 `192.168.1.128:9800` 不提供 embedding 模型→退化纯 BM25，已不含 mock 向量）；真实数据全程未损（2 条真实记忆 `04f36e6b8894`/`7e489b80c582`、7 个真实页、10 个真实会话、`providers.cm_8d81ae73`、`.env` 均核对一致）。**验证（重启后连跑两轮完整 verify）**：`py_compile` 三文件 OK；服务重启（mock 4831 PID 26236 / server 4830 PID 58320），重启后 `/api/jobs` 归零、`DELETE /api/jobs/nonexistent` 返 404（端点存在）；两轮跑完逐字段 diff before/after——sessions/pages/jobs/memories/automations/hello.txt/rag(embed='',119)/config(base_url+model+roles+workflow) **全部 OK，ZERO RESIDUE=True**；settings+base_url 还原精确（base_url 回内网代理、model 回空、roles 回 deepseek）。**已知失败（非本轮引入，未修）**：S11 RAG 拒答断言 FAIL——拒答探针查询 `quantum entanglement bell inequality`（英文）与本轮 S2/S7 注入的短英文 mock 页（`hello from mock executor`）共享 bigram（`el`/`ll`），短文档 BM25 长度归一化把 `best_bm25/6.0` 顶过 `refuse_threshold=0.35`→未拒答；向量路径无辜（实测 cos 全 <0.35：mock 页 0.12/中文 0.18）；对干净 119-chunk 索引同查询 conf=0.0 正确拒答，证明是「测试自污染语料打破自身拒答断言」，确定性复现、与本轮改动无关。13/14 PASS（仅 S11）。**文档**：§3.7 端点数 66→67 + jobs 行补 DELETE、§6 离线验收行补自清扫与 S11 已知失败、§9 本行、§10 刷新 PID/jobs/RAG/config。**需重启**（改 `app/job_scheduler.py`+`app/routes/extras.py`，已重启生效）。 |
| v1.0.19 | 2026-09-25 | Qoder | **`scripts/verify.py` 配置自快照/自恢复，杜绝崩溃后残留 mock 坏配置**。痛点：此前 S0 之后内联 `PUT /api/settings` 把 model + 4 角色切成 mock（mock-planner/executor/evaluator/memory），**全程不还原**——一旦中途断言失败、异常或 Ctrl-C，`data/config.json` 就停在 mock 态，靠人记得手动切回，极脆弱（实测当前 live config 的 `llm.model` 仍是上一轮残留的 `mock-executor`，使无 override 的角色 executor_recovery/doc_*/multimodal 都解析到 mock）。改法：① 抽出 `MOCK_SETTINGS` 常量替换内联字典；② 新增 `snapshot_settings()`——**直接读 `config.json` 原文**而非 GET `/api/settings`，因为后者经 `model_router.describe()` 返回的是解析后的角色（model 已逐级回退、空值显示为「(未配置)」），据此无法精确还原；③ 新增 `restore_settings(snap)`——**走 PUT 而非直接写盘**，因 `save_config()` 会同步刷新服务端进程内 `_config` 缓存，免重启即生效（直接改文件运行中的服务读不到）；只还原 mock 实际改动的字段（model / 4 角色 provider+model / workflow.mode / evaluator.enabled / memory.enabled），其余字段未被动过故不碰；④ `__main__` 用 `try: sys.exit(main()) finally: restore_settings(_SNAPSHOT)` 兜底，`_SNAPSHOT` 由 main 切换前写入、未切换时为 None 使恢复成 no-op（S0 失败早返回不会误报警告）。**验证（针对 live 服务 4830，未跑全套 13 阶段以免污染 live 记忆/页面/任务）**：`py_compile` OK；happy-path 往返——snapshot→PUT mock（executor 实测变 `mock-executor`）→restore，4 角色 + model + workflow + evaluator + memory **逐字段精确还原**（`after == before` True）；崩溃路径——切换后抛 `KeyboardInterrupt`，`finally` 仍把 executor 还原回 `deepseek-v4-flash`（True）。**文档**：§3.7 verify.py 行与 §6 离线验收行补「自快照/自恢复」说明、§9 新增本行。**遗留（已清除）**：live `config.json` 历史残留的 `llm.model='mock-executor'`（上一轮崩溃留下、使 executor_recovery/doc_*/multimodal 误解析到 mock）经用户确认后用 `PUT /api/settings {model:''}` 改回空字符串（磁盘 + 服务端缓存均已生效，executor_recovery 实测回退真实 `cm_8d81ae73`/`deepseek-v4-flash`）；本补丁的恢复只还原到「运行前状态」，故这条运行前就已存在的坏值需单独清理。 |
...
## 10. 当前运行状态与待办（易失分区，状态一变即改；2026-09-25 23:45 实测）

- 服务在跑：http://127.0.0.1:4830，监听进程 **PID 58320**（2026-09-25 由 `scripts\_dev_start_server.ps1` 重启，加载 v1.0.20 的 `app/job_scheduler.py`+`app/routes/extras.py` 改动）；mock 网关 4831 **PID 26236**（`scripts\_dev_launch_mock.ps1`）。**一律以 `Get-NetTCPConnection -LocalPort 4830` 为准**（`Start-Process` 回显 PID 可能与监听 PID 不同）；日志重定向到 `data\cache\server.out.log` / `server.err.log`（uvicorn 走 stderr，中文经控制台读出是 GBK 乱码但文件本身是 UTF-8）；停止用 `scripts\_dev_stop_server.ps1`（注意它会**连 4831 mock 网关一起杀**，只需停主服务时用 `Stop-Process -Id <监听PID>`），正式使用/重启 `.\start.ps1` 或 `start.bat`
...
- **前端当前 bundle：`index-NOIfNOfL.js`（449.67 kB / gzip 128.91 kB，构建于 2026-09-26）+ `index-BaHgLdt7.css`（89.48 kB / gzip 15.58 kB）**，`web/dist/index.html` 已引用（v1.0.26 全应用 i18n：中/韩/英实时切换）
- 用户数据现状（2026-09-25 23:45 实测，v1.0.20 清理后）：会话 **11 个**、资料库页 **7 个**（SAP/BOM 报告 + RAG 说明 + Arche 使用手册 + 经济/库存等真实页）、记忆 **2 条**（均 auto：`04f36e6b8894`「偏好直白的数据结论…」、`7e489b80c582`「每月用 Excel…挤出线废料」）、定时任务 **0 个**、自动化 **1 个**。本轮已彻底清除历史 mock 测试污染：3 条 mock 记忆（`用户喜欢蓝色`/`用户偏好简洁回答`/`去重测试条目`）、2 个 mock 页（`hello`/`mock 页面`）、2 个 verify 会话、9 个 `mock 提醒` 任务、`workspace/hello.txt`、以及被 S11 用 mock 网关重建的 RAG 索引（`mock-embed`/121chunks → 已 reindex 为 `(bm25)`/119chunks）。`data/config.json` 的 `llm.model` 历史残留 `mock-executor` 已改回**空**（v1.0.19 补丁不会自动清运行前就存在的坏值，单独经 PUT 修正）。jobs 清理备份在 `data/cache/jobs.json.pre-mock-clean`
- **网关/角色配置已变更（2026-09-25 实测，覆盖 528/530 行旧叙述）**：`llm.base_url = http://192.168.1.128:9800/v1`（内网代理）仍为 default，但 **planner/executor/evaluator/memory 四角色的 `provider` 现为自定义 `cm_8d81ae73`（deepseek，`https://api.deepseek.com/v1`，model `deepseek-v4-flash`）**，且 `.env` 的 `CONN_CM_8D81AE73_KEY` **已设置**（35 字符，此前 09-11 记的「未设置/无可用回退」已作废）；`API_KEY`（内网代理）亦在 `.env`。`llm.model` 留空、`rag.embed_model` 留空（内网代理不提供 embedding 模型，RAG 实际跑纯 BM25）
...
- **离线验收**：`scripts/verify.py` + `scripts/mock_llm.py`（4831）。2026-09-05 首次全绿 11/11；v1.0.5 扩至 14 项（S0-S13）。跑前须先起 mock；**v1.0.19 起 settings 自快照/自恢复**（切换前读 config.json 存底，`try/finally` 经 PUT 精确还原，崩溃/Ctrl-C 也不留坏配置）；**v1.0.20 起 base_url 自切换 + 数据残留自清扫**（`MOCK_SETTINGS` 含 base_url 指向 mock 网关并还原；finally 按基线删本轮新增的会话/记忆 + mock 标识的页/任务 + hello.txt，并 reindex 复位 RAG），连跑实测 ZERO RESIDUE；**v1.0.21 修 S11 拒答探针**（英文→中文「量子纠缠贝尔不等式」，避开与英文 mock 页的 bigram 重叠）。**当前 14/14 全 PASS**。
